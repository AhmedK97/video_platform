...
class ConvertVideoForStreaming implements ShouldQueue
{
    ...
    public function convertVideo($loopNumber)
    {
        $this->format = array(
            array(
                (new X264('libmp3lame', 'libx264'))->setKiloBitrate(4096), (new WebM('libvorbis', 'libvpx'))->setKiloBitrate(4096)
            ),
            array(
                (new X264('libmp3lame', 'libx264'))->setKiloBitrate(2048), (new WebM('libvorbis', 'libvpx'))->setKiloBitrate(2048)
            ),
            array(
                (new X264('libmp3lame', 'libx264'))->setKiloBitrate(750), (new WebM('libvorbis', 'libvpx'))->setKiloBitrate(750)
            ),
            array(
                (new X264('libmp3lame', 'libx264'))->setKiloBitrate(500), (new WebM('libvorbis', 'libvpx'))->setKiloBitrate(500)
            ),
            array(
                (new X264('libmp3lame', 'libx264'))->setKiloBitrate(300), (new WebM('libvorbis', 'libvpx'))->setKiloBitrate(300)
            ),
        );
    ...
    public function handle()
    {
        ...
        if ($width > $height) {
            if (($width >= 1920) && ($height >= 1080)) {
                ...
                $quality = 360;
                $this->convertVideo(3);
            } elseif (($width >= 426) && ($height >= 240) && ($width < 640 && $height < 360)) {
                $quality = 240;
                $this->convertVideo(4);
            }
            // المقطع الطولي
            ...
            if (($width >= 1080) && ($height >= 1920)) {
                $quality = 1080;
                $this->convertVideo(0);
            } elseif (($width >= 720) && ($height >= 1280) && ($width < 1080 && $height < 1920)) {
                $quality = 720;
                $this->convertVideo(1);
            } elseif (($width >= 480) && ($height >= 854) && ($width < 720 && $height < 1280)) {
                $quality = 480;
                $this->convertVideo(2);
            } elseif (($width >= 360) && ($height >= 640) && ($width < 480 && $height < 854)) {
                $quality = 360;
                $this->convertVideo(3);
            } else {
                $quality = 240;
